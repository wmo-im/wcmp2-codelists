...
readme_tpl = Template("""**Project name:** WCMP2 Codetables
**Description:** This directory contains the WCMP2 Codetables. 
**Table of Contents:**
$payload
""")
...
def generate(dir=None):    
    # get codelist to 
    codes = {}
    with open(r"{}/wcmp2-tables.csv".format(TABLES_DIR),'r') as f:
...
def createReadme():
    # get codelist to 
    codes = {}
    with open(r"{}/wcmp2-tables.csv".format(TABLES_DIR),'r',encoding="utf8") as f:
        csvreader = csv.reader(f)     
        files = []
        temp = ""
        for line in csvreader:
    ...
    if len(missing_files)>0:
        raise ValueError("{} are referenced in wcmp2-tables.csv but do not exist ".format( ",".join([ "{}.csv".format(f) for f in missing_files ] ) ))        
    if len(missing_in_csv)>0:
        raise ValueError("{} are in filesystem but not referenced in wcmp2-tables.csv ".format( ",".join( [ "{}.csv".format(f) for f in missing_in_csv ] ) ) )
